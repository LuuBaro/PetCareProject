...
public class VNPayController {
    ...
    @Autowired
    private OrderService orderService; // Tiêm OrderController
    ...
    // Phương thức xác nhận thanh toán và lưu đơn hàng vào SQL
    @PostMapping("/confirmPayment")
    public ResponseEntity<Map<String, String>> confirmPayment(@RequestBody OrderController.CheckoutRequest request) {
        try {
            // Chỉ thực hiện lưu đơn hàng vào cơ sở dữ liệu nếu thanh toán VNPay thành công
            orderService.processOrder(request);

            Map<String, String> response = new HashMap<>();
            response.put("message", "Đơn hàng đã được lưu thành công.");
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            Map<String, String> errorResponse = new HashMap<>();
            errorResponse.put("error", "Lưu đơn hàng thất bại");
            errorResponse.put("message", e.getMessage());
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResponse);
        }
    }
